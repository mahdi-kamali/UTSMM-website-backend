...
router.post("/tickets", async (req, res, next) => {
  try {
    ...
    const { subject, orderID, request, message } = await req.body;
...
router.post("/order", async (req, res, next) => {
  try {
    ...
    const { link, serviceID, quantity, charge } = req.body;
  ...
  try {
    ...
    platforms.forEach((platform) => {
      ...
      const temp = activeOrders.filter((item) => {
        return item.service.name.toLowerCase().trim().includes(platformName);
      });
...
router.get("/statistics/saved-services", async (req, res) => {
  ...
  const temp = userSavedServices
    .map((item, index) => {
      ...
      if (!service) {
        console.log(
          `isNot Valid ${service}  index = ${index} service = ${item}`
        );
      } else return service;
    })
    .filter((item) => {
  ...
  user.savedServices = temp.map((item) => {
    return item.service;
  });
...
router.post("/statistics/saved-services", async (req, res) => {
  ...
  const service = require("../../../services.json").find((service) => {
...
router.post("/gift/trust-pilot/submit-proof", async (req, res, next) => {
  try {
    ...
    return res.json(
      "Your Proof Successfuly Submited Please wait for validation."
    );
  } catch (e) {
    if (e?.code === 11000) {
...
router.get("/affliates", async (req, res, next) => {
  try {
    ...
    const revenue = {
      daily: dailyRecords.reduce(
        (total, record) => total + parseFloat(record.ammountForGift),
        0
      ),
      weekly: weeklyRecords.reduce(
        (total, record) => total + parseFloat(record.ammountForGift),
        0
      ),
      yearly: yearlyRecords.reduce(
        (total, record) => total + parseFloat(record.ammountForGift),
        0
      ),
    };
